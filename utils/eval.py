...
def get_dataset_scores(mask_root, scores, metadata, max_clip=None, scene_id=None, save_results=False, directory="results/chad_score_normal_dist/"):
    dataset_gt_arr = []
    dataset_scores_arr = []
...
def score_align(scores_np, gt, seg_len=30, sigma=40):
    ...
    return auc_roc, shift, sigma, auc_precision_recall, EER, eer_threshold
...
def get_train_dist(model, train_loader, args=None):
    param_list = []
    with torch.no_grad():
        for itern, data_arr in enumerate(tqdm(train_loader)):
            model.eval()
        ...
        # m = np.array(param_list)[:, :, 0]
        # v = np.array(param_list)[:, :, 1]
        
        mean = np.mean(param_arr, axis=0)
        std = np.std(param_arr, axis=0)
