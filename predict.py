...
def run(**kwargs):
    ...
    if fake_args.model_dist == 'normal':
        prior_dist = dist.Normal()
        q_dist = dist.Normal()
    elif fake_args.model_dist == 'laplace':
        prior_dist = dist.Laplace()
        q_dist = dist.Laplace()
    elif fake_args.model_dist == 'flow':
        prior_dist = FactorialNormalizingFlow(dim=fake_args.latent_dim, nsteps=32)
        q_dist = dist.Normal()

    if model_backend == "torch":
        model = VAE(z_dim=fake_args.latent_dim, 
            use_cuda=True, 
            device=device,
    ...
    with open(pathf_tsgad_mean_normal, "rb") as f:
        mean_normal = pickle.load(f)
    mean_normal = torch.from_numpy(mean_normal).to(device)
    ...
    for namef_lbl in tqdm(sorted(os.listdir(pathd_lbl))):
        ...
        for i_obj, (box_xcycwhn, kpts_xyn, id_track) in enumerate(
            ...
            with torch.no_grad():
                # rescale to [-1, 1]
                seq_kpts = (seq_kpts[..., :2] - seq_kpts[..., :2].mean(axis=(1, 2))[:, None, None, :]) / seq_kpts[..., 1].std(axis=(1, 2))[:, None, None, None]
                pts1 = torch.tensor(seq_kpts, dtype=torch.float32).permute(0, 3, 1, 2)
                
                mtime_1 = time.time()
                if model_backend == "torch":
                    pts1 = pts1.to(device, non_blocking=True)
                    _, z_params, _ = model.encode(pts1)
                    z_params = z_params.view(z_params.shape[0], -1)
                    l2_distance = (torch.sqrt(torch.sum((z_params - mean_normal)**2, dim=1))).cpu().numpy()[0]
                    prob = l2_distance
                elif model_backend == "tensorrt":
                    raise NotImplementedError("TensorRT backend is not implemented yet.")
                mtime_2 = time.time()
