...
class GraphEncoder(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.hidden_dim = (
            (input_frames / downsample_factor) * num_node * h_dim * self.fig_per_seq
        )
    ...
    def build_enc(self, enc_ch_fac, enc_stride):
        ...
        enc_kwargs[-1] = {
            "out_act": False,
            **enc_kwargs[-1],
        }  # No Relu for final encoder layer
        st_gcn_enc = [
            ConvBlock(
                self.in_channels,
                enc_ch_fac[0] * self.h_dim,
                self.kernel_size,
                enc_stride[0],
                **enc_kwargs[0]
            )
        ...
        for i in range(1, len(enc_ch_fac)):
            st_gcn_enc.append(
                ConvBlock(
                    enc_ch_fac[i - 1] * self.h_dim,
                    enc_ch_fac[i] * self.h_dim,
                    self.kernel_size,
                    enc_stride[i],
                    **enc_kwargs[i]
                )
...
class GraphDecoder(nn.Module):
    ...
    def forward(self, z, x_size):
        ...
        x, _ = self.dec_final_gcn(
            x, self.A * self.ei_dec[-1]
        )  # Final layer has no upsampling
        if self.fig_per_seq == 1:
            return x
    ...
    def build_dec(self, dec_ch_fac, dec_stride, dec_1st_residual=True):
        ...
        for i in range(1, len(dec_ch_fac)):
            if dec_stride[i] != 1:
                st_gcn_dec.append(
                    nn.Upsample(scale_factor=(dec_stride[i], 1), mode="bilinear")
                )

            st_gcn_dec.append(
                ConvBlock(
                    dec_ch_fac[i - 1] * self.h_dim,
                    dec_ch_fac[i] * self.h_dim,
                    self.kernel_size,
                    1,
                )
            )
...
class VAE(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.mu = nn.Linear(
            int(self.encoder.hidden_dim / self.q_dist.nparams),
            int(self.encoder.hidden_dim / self.q_dist.nparams),
        )
    ...
    def encode_v2(self, x):
        ...
        # sample the latent code z
        zs = z_params[:, :, 0] + torch.randn_like(z_params[:, :, 1]) * torch.exp(
            0.5 * z_params[:, :, 1]
        )  # reparameterization trick, assuming mu and logvar
        if self.graph:
            return zs, z_params, x_size
        else:
            return zs, z_params
    ...
    def decode(self, z, x_size=None):
        if self.graph:
            x_params = self.decoder.forward(z, x_size).view(
                z.size(0), 2, self.input_frames, self.num_kpts
            )  # hardcoded a bit N, C, T, V, M
        else:
            x_params = self.decoder.forward(z).view(z.size(0), 1, 64, 64)
        xs = self.x_dist.sample(
            params=x_params
        )  # Why do you sample again? the output of the decoder is distribution parameters> Why?
        return xs, x_params
    ...
    def elbo(self, x, dataset_size):
        ...
        if self.graph:
            _logqz = self.q_dist.log_density(
                zs.view(
                    batch_size, 1, int(self.encoder.hidden_dim / self.q_dist.nparams)
                ),
                z_params.view(
                    1,
                    batch_size,
    ...
    def fn_loss_kl(self, **kwargs):
        ...
        fn = eval("torch.{}".format(reduction))
        return 0.5 * fn(
            target_logvar
            - logvar
            + (logvar.exp() + (mu - target_mu).pow(2)) / target_logvar.exp()
            - 1
        )
    ...
    def elbo_v2(
        ...
        x_class1,
        target_mu_class1,
        target_logvar_class1,
        x_class2,
        target_mu_class2,
        target_logvar_class2,
    ):
        # log p(x|z) + log p(z) - log q(z|x)
        ...
        if self.graph:
            x_class1 = x_class1.view(
                batch_size_class1, 2, self.input_frames, self.num_kpts
            )
            x_class2 = x_class2.view(
                batch_size_class2, 2, self.input_frames, self.num_kpts
            )
        else:
        ...
        # prior_params = self._get_prior_params(batch_size)
        xs_class1, x_params_class1, zs_class1, z_params_class1 = (
            self.reconstruct_img_v2(x_class1)
        )
        xs_class2, x_params_class2, zs_class2, z_params_class2 = (
        ...
        loss_kl_class1 = self.fn_loss_kl(
            mu=mu_class1,
            logvar=logvar_class1,
            target_mu=torch.tensor(target_mu_class1, device=self.device),
            target_logvar=torch.tensor(target_logvar_class1, device=self.device),
            reduction="mean"
        )
        loss_kl_class2 = self.fn_loss_kl(
            mu=mu_class2,
            logvar=logvar_class2,
            target_mu=torch.tensor(target_mu_class2, device=self.device),
            target_logvar=torch.tensor(target_logvar_class2, device=self.device),
            reduction="mean"
        )
        ...
        # print("--- Loss class 2: {:.3f}, {:.3f}".format(loss_rec_class2.item(), loss_kl_class2.item()))

        # laptq: returning loss (i.e., to minimize) instead of elbo (i.e., to maximize)
        return {
            "loss": loss,
            "loss_rec": loss_rec,
            "loss_kl": loss_kl,
