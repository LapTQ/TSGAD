...
from utils.data_utils import normalize_pose, normalize_pose_v2
...
class PoseSegDataset(Dataset):
    ...
    def __init__(
        ...
        **dataset_args
        ...
        if abnormal_train_path is not None:
            (
                self.segs_data_np_ab,
                self.segs_meta_ab,
                self.person_keys_ab,
                self.global_data_np_ab,
                self.global_data_ab,
                self.segs_score_np_ab,
            ) = gen_dataset(
                abnormal_train_path,
                num_clips=num_clips,
                ret_keys=True,
            ...
            self.segs_data_np = np.concatenate(
                (self.segs_data_np, self.segs_data_np_ab), axis=0
            )
            self.segs_meta = np.concatenate((self.segs_meta, self.segs_meta_ab), axis=0)
            self.global_data_np = np.concatenate(
                (self.global_data_np, self.global_data_np_ab), axis=0
            )
        ...
        self.person_keys = {
            k: [int(float(i)) for i in v] for k, v in self.person_keys.items()
        }
    ...
    def __getitem__(self, index):
        ...
        if self.normalize_pose_segs:
            data_transformed = (
                normalize_pose_v2(
                    data_transformed.transpose((1, 2, 0))[None, ...], **self.args
                )
                .squeeze(axis=0)
                .transpose(2, 0, 1)
            )
    ...
    def get_all_data(self, normalize_pose_segs=True):
        if normalize_pose_segs:
            segs_data_np = normalize_pose_v2(
                self.segs_data_np.transpose((0, 2, 3, 1)), **self.args
            ).transpose((0, 3, 1, 2))
        else:
...
def get_dataset_and_loader(args, trans_list, only_test=False, skip_testset=False):
    ...
    for split in splits:
        evaluate = split == "test"
        abnormal_train_path = (
            args.pose_path_train_abnormal if split == "train" else None
        )
        normalize_pose_segs = args.global_pose_segs
        dataset_args["trans_list"] = (
            trans_list[: args.num_transform] if split == "train" else None
        )
        dataset_args["seg_stride"] = (
...
def gen_dataset(
    ...
    **dataset_args
    ...
    if (
        ...
        json_list = sorted(
            [fn for fn in dir_list if fn.endswith("tracked_person.json")]
        )
    ...
    for person_dict_fn in tqdm(json_list):
        ...
        _, _, _, global_data_np, global_data, _ = gen_clip_seg_data_np(
            clip_dict,
            start_ofst,
            1,
            1,
    ...
    if seg_conf_th > 0.0:
        segs_data_np, segs_meta, segs_score_np = seg_conf_th_filter(
